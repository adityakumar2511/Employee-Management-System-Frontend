...
import { settingsService } from "@/services/settings.service"
import { useToast } from "@/hooks/useToast"
import { MapPin, Plus, Trash2, Edit, Navigation, Building, ExternalLink } from "lucide-react"
import { ConfirmDialog } from "@/components/ui/ConfirmDialog"
...
export default function GeoSettingsPage() {
  ...
  const addMutation = useMutation({
    mutationFn: settingsService.addGeoLocation,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["geo-locations"] })
      toast.success("Office location added!")
      setAddModal(false)
      setForm({ name: "", address: "", latitude: "", longitude: "", radius: 500 })
    },
    onError: (e) => toast.error(e.response?.data?.message || "Failed to save"),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (selectedLocation?.id === deleteLocation?.id) setSelectedLocation(null)
    },
  })
  ...
  return (
    <AdminLayout title="Geo-Fence Settings">
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="page-header mb-0">
            ...
            <p className="page-subtitle">
              Configure office locations and attendance radius
            </p>
          </div>
          <Button size="sm" onClick={() => setAddModal(true)}>
            <Plus className="h-4 w-4" /> Add Location
        ...
        <div className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800 p-4">
          ...
          <div className="text-sm">
            ...
            <p className="text-blue-600 dark:text-blue-400 mt-0.5">
              Employees can only check-in when within the configured radius of
              an office location. Browser GPS is used — no extra hardware
              needed.
            </p>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                {activeLocation ? (
                  <>
                    <LocationPicker
                      center={{
                        ...
                        lng: parseFloat(activeLocation.longitude),
                      }}
                      radius={activeLocation.radius}
                      height="280px"
                      readonly
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-3">
                      <div className="rounded-lg bg-muted/40 p-2 text-center">
                        <p className="font-mono text-xs">
                          {parseFloat(activeLocation.latitude).toFixed(4)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Latitude
                        </p>
                      ...
                      <div className="rounded-lg bg-muted/40 p-2 text-center">
                        <p className="font-mono text-xs">
                          {parseFloat(activeLocation.longitude).toFixed(4)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Longitude
                        </p>
                    ...
                    <div className="mt-3 flex justify-end">
                      <a
                        href={`https://www.google.com/maps?q=${activeLocation.latitude},${activeLocation.longitude}&z=16`}
                        target="_blank"
                  ...
                  <div className="h-64 flex items-center justify-center rounded-xl border border-dashed text-muted-foreground">
                    <div className="text-center">
                      <MapPin className="h-10 w-10 mx-auto mb-2 opacity-20" />
                      <p className="text-sm">No location selected</p>
                      <p className="text-xs mt-1">
                        Click a location from the list
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Employees outside this radius cannot check-in. Default: 500m
            </p>
          </div>
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditLocation((f) => ({ ...f, name: e.target.value }))
                  }
                  className="mt-1.5"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditLocation((f) => ({ ...f, address: e.target.value }))
                  }
                  className="mt-1.5"
                />
              </div>
          ...
          <div className="flex gap-3 justify-end mt-6">
            ...
            <Button
              ...
              onClick={() => updateMutation.mutate(editLocation)}
              disabled={
                !editLocation.name ||
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteMutation.mutate(deleteLocation?.id)}
        loading={deleteMutation.isPending}
        title="Delete Location"
        message={`Delete "${deleteLocation?.name}"? Employees assigned here won't be able to check-in.`}
        confirmLabel="Delete"
      />
    </AdminLayout>
  )
